...
export function createTeammateTool(swarmManager: SwarmManager | null): ToolDefinition {
  return tool({
    ...
    execute: async (args) => {
      ...
      if (!args.dry_run) {
        if (!swarmManager) {
          return formatTeammate({ success: false })
        }
        const prompt = `You are teammate "${teammateName}" on team "${teamName}". Check your inbox for task assignments.`
        await swarmManager.spawnTeammate(teammateName, prompt, {
          teamName,
          mode,
        })
      }
